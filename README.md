...
**Geneerinen ingest-arkkitehtuuri on valmis (`MockCollector`).** SQLite, repository, merge-engine ja yhteyshenkilötuki ovat valmiit. Oikeita datalähteitä ei ole vielä kytketty. Katso [sources.md](sources.md) ja [roadmap.md](roadmap.md).
...
| Modulaariset ingestorit | ✅ | `BaseCollector`, `HttpClient`, `JsonFeedCollector`, `MockCollector` |
...
│       ├── collectors/       # Ingestorit (BaseCollector, MockCollector, …)
...
Geneerinen ingest-arkkitehtuuri: **BaseCollector** → **CollectorResult** → **ClubObservation** → merge-engine.

| Kerros | Tiedosto | Kuvaus |
|--------|----------|--------|
| `MockCollector` | `collectors/mock.py` | 3 seuraa muistista (CLI-oletus) |
| `HttpClient` | `collectors/http/client.py` | Retry, timeout, rate limit, User-Agent |
| `HttpCollector` | `collectors/http/base.py` | HTTP/JSON-pohjaisten ingestorien pohja |
| `JsonFeedCollector` | `collectors/examples/json_feed.py` | Esimerkki HTTP JSON -syötteestä |

```bash
urheiluseurapro sources
urheiluseurapro collect mock --format json
```

HTTP-esimerkki (rekisteröi collector erikseen):

```python
from urheiluseurapro.collectors import JsonFeedCollector, CollectorRegistry

registry = CollectorRegistry()
registry.register(JsonFeedCollector())
```

Keruu tuottaa `CollectorResult`-olioita, joista jokainen sisältää:
- `source`, `source_url`, `confidence`, `fetched_at`, `first_seen_at`, `last_seen_at`
- `ClubObservation`-havainnon (seura + yhteyshenkilöt) merge-engineä varten

Ei HTML-scrapausta, ei selainautomaatiota – vain suorat HTTP-pyynnöt strukturoituun dataan (JSON/API).

Testit: `pytest tests/test_collectors.py tests/test_http_client.py tests/test_json_feed_collector.py`
